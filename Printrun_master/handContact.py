...
def add_new_p(gcode_list: list, direction : str):
    ...
    gcode_list.insert(2, f"G1 X{int(direction == 'Left') * '-'}1 Y0.1 F3000 ;")
    gcode_list.insert(3, "G1 Z0.5 ;")
    print(gcode_list[0:7])
...
if __name__ in "__main__":
    ...
    if input_answer == 'y':
        ...
        # print first layer
        print('first layer')
        gcode = [i.strip() for i in open('CE3E3V2_umpty_r.gcode')]
        gcode = gcoder.LightGCode(gcode)
        p.startprint(gcode)
        time.sleep(0.5)

        # start continues printing
        layer_template = [i.strip() for i in open('CE3E3V2_umpty_r_rel.gcode')]
        layer_number = 1
